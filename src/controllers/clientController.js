...
export const uploadClientSingleDocument = async (req, res) => {
  try {
    const applicationId = req.params.id;
    const client = req.client;
    if (!client) return res.status(401).json({ message: "Not authenticated" });
    ...
    if (!fieldName) return res.status(400).json({ message: "fieldName is required in body (e.g. 'paymentReceipt')" });

    const file = req.file;
    if (!file) return res.status(400).json({ message: "No file uploaded (use form field 'file')" });
    ...
    if (!fileUrl) return res.status(500).json({ message: "Could not obtain uploaded file URL" });

    const updateData = {};
    updateData[`documents.${fieldName}`] = fileUrl;

    const updated = await Application.findByIdAndUpdate(applicationId, { $set: updateData }, { new: true });
    return res.json({ message: "Document uploaded", documents: updated.documents });
  } catch (err) {
    console.error("uploadClientSingleDocument error:", err);
    res.status(500).json({ message: err.message });
  }
};
...
// also export single-document handler as named export for routes that prefer it
export { uploadClientSingleDocument };
